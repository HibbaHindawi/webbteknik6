...
//Hämta alla listor
$app->get('/lists/user/{userId}', function ($request, $response, $args) {
...
$app->post('/lists/{id}', function ($request, $response, $args) {
    $id = intval($args['id']);
    $result = queryMySQL("DELETE FROM Lists WHERE id='$id'");
...
$app->get('/lists/{listId}/tasks', function ($request, $response, $args) {
...
$app->post('/lists/{id}/tasks', function ($request, $response, $args) {
    ...
    $creatorID = intval($data['userId']);
    $desc = sanitizeString($data['description']);
    $points = sanitizeString(intval($data['points']));
    $insertQuery = "INSERT INTO Tasks (name, listId, userId, description, points) VALUES('$taskName', '$listId', '$creatorID', '$desc', '$points')";
    $result = queryMySQL($insertQuery);
    if ($result) {
...
$app->post('/tasks/{id}/status', function ($request, $response, $args) {
    ...
    $query = $taskStatus === 1 ? "UPDATE Tasks SET status = $taskStatus, completeUser = $userId WHERE id = $taskId" : "UPDATE Tasks SET status = $taskStatus, completeUser = NULL WHERE id = $taskId";
...
$app->post('/tasks/{id}', function ($request, $response, $args) {
    $taskId = intval($args['id']);
    $result = queryMySQL("DELETE FROM Tasks WHERE id='$taskId'");
    if ($result) {
        return $response->withJson(["success" => true]);
    }
...
$app->get('/lists/{id}/users', function ($request, $response, $args) {
    $id = intval($args['id']);
    $users = [];
    $result = queryMySQL("SELECT Users.username, COALESCE(SUM(Tasks.points), 0) AS total_points FROM ListMembers JOIN Users ON ListMembers.userId = Users.id LEFT JOIN Tasks ON Tasks.completeUser = Users.id AND Tasks.listId = ListMembers.listId WHERE ListMembers.listId = '$id' GROUP BY Users.id, Users.username");
    while ($row = $result->fetch_assoc()) {
        $users[] = $row;
    }
...
$app->get('/lists/{listId}/members/{userId}', function (Request $request, Response $response, $args) {
